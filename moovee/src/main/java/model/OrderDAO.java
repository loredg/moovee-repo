...
public class OrderDAO implements IBeanDAO<Order> {
	...
	public void doSave(Order order) throws SQLException {
		...
		try {
			...
			ps = connection
					.prepareStatement("INSERT INTO ordine(totale, data, idAccount, idIndirizzo) VALUES(?, ?, ?, ?)");
	...
	public Collection<Order> doRetrieveAllByUser(String userId, String order) throws SQLException {
		Connection connection = null;
		PreparedStatement ps = null;

		Collection<Order> orders = new LinkedList<Order>();
		String selectSQL = "SELECT * FROM ordine WHERE idAccount = ?";
